# 手势词典学习与设计评估 Agent：从用户输入到回答的处理流程

本文档用于说明当前 `gesture_agent` 原型的工作链路，重点解释用户输入如何被拆解、如何检索 `data/` 中的手势词典内容，以及如何通过硅基流动 API 生成最终回答或设计评估建议。

## 1. 当前目标

当前版本是“手势词典学习与设计评估 Agent”的原型，目标不是做普通聊天，而是辅助用户按手势词典的结构学习交互设计知识，并评价用户提供的设计方案。
...
11. 设计方案评估：例如“请评估这个方案是否合理，并用手势词典结构给修改建议”

核心原则：

- 先理解问题结构，再调用模型回答。
- 回答必须使用手势词典的结构语言：控件形态、基本属性、交互机制、响应逻辑、交互特性、适用/不适用场景。
- 评估设计方案时，先规范术语复述，再诊断机制冲突、反馈缺失和适用边界问题。
...
flowchart TD
    ...
    D --> D1{是否为设计方案评估}
    D1 -->|是| D2[DesignParser 抽取控件、属性、机制、反馈、风险]
    D1 -->|否| E[KnowledgeBase 检索相关词典片段]
    D2 --> E
...
| 设计评估拆解 | `gesture_agent/evaluation/design_parser.py` | 从设计方案中抽取控件形态、基础属性、交互机制、系统反馈和风险点 |
...
| `design_evaluation` | 评估/评价/设计方案/合理吗/优化建议 | 方案复述、结构拆解、问题诊断、修改建议、规范术语版本、需要补充的信息 |

设计方案评估会额外挂载 `design_evaluation` 字段：

```json
{
  "raw_proposal": "用户原始方案",
  "modality": ["text"],
  "product_context": "产品/界面/场景",
  "user_goal": "用户目标",
  "control_forms": ["旋钮"],
  "basic_properties": ["位置属性"],
  "mechanisms": ["长按", "拖动"],
  "system_feedback": ["高亮", "确认"],
  "risk_points": ["点击与拖拽共存时，需要定义点拖互斥或位移阈值。"],
  "missing_info": []
}
```
...
检索逻辑是本地混合检索，不调用外部向量数据库：

1. 术语别名归一化：例如“点一下”会先映射为“单击”，“按住”会先映射为“长按”。
2. Query Rewrite：根据结构化知识库把命中的术语扩展成“术语类型、属性、机制、控件形态、关联术语、响应逻辑”等检索词。
3. 关键词打分：术语命中标题权重大于正文命中；标题 token 重合高于正文 token 重合。
4. 轻量向量相似度：对改写后的 query 和 chunk 的标题、正文、结构化字段做 token set Jaccard 相似度，作为本地向量近似分。
5. 结构化 rerank：如果 chunk 对应的结构化条目命中当前术语、属性、机制或控件形态，会额外加权。
6. `interaction_mechanism` 会略微加权，因为当前核心学习内容主要是交互机制。
7. `data/交互机制对比.md` 会按“开关 vs 单击 vs 按下”“快击 VS 点击缓冲”等编号小节切块，并从标题中拆出对比对象术语。
8. 对结果去重，保留排名最高的 `top-k` 条。
