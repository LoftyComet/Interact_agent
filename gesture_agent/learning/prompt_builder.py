...
DEFAULT_RESPONSE_INSTRUCTIONS = [
    ...
    "交互机制：在“核心定义”一节里必须点明该机制属于哪一类交互机制、对应词典的哪个章节（如检索资料标题所示）；依次讲清基础属性、响应逻辑、交互特性、典型案例、适用与不适用、关联机制。“交互特性”优先使用词典原有的响应速度、操作难度、容错性、可发现性等维度，不要擅自改写成泛化的收益与代价分析。",
    ...
    "功能交互拆解中的证据边界：用户描述的产品行为只是待分析输入，不是词典事实。语料块只写资料直接支持的机制定义和书中案例；把产品功能映射到某个机制属于分析推导，必须完整放入 design_reasoning 块并使用待验证措辞。",
    ...
    "澄清问题只询问尚缺的信息；不要在问题选项或括号说明中夹带未经本轮资料支持的用户偏好、场景规律或产品事实。",
...
def format_available_images(images: Optional[list["ImageEntry"]]) -> str:
    ...
    for img in images:
        lines.append(
            f"- image:{img.reference_id} — {img.annotation}（章节：{img.heading}）"
        )
    return "\n".join(lines)
