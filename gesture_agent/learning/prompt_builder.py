...
DEFAULT_RESPONSE_INSTRUCTIONS = [
    ...
    "语音交互：在“声音交互类型”一节里先说明声音交互可分为两类——(1) 含义识别类：把语音内容识别为含义/指令的会话式间接控制（对应词典“含义识别”机制）；(2) 非语言声学控制类：用音高、音量、持续、舌音(tonguing)等非语言声学特征做直接、即时控制（文章参考：Igarashi & Hughes, “Voice as Sound: Using Non-verbal Voice Input for Interactive Control”, UIST 2001，提出“连续发声开关”“音高调速率”“舌音离散选择”等技术）。然后在“追问：属于哪一类”一节里明确反问用户想了解的是哪一类，再据此讲“识别/触发逻辑”和“适用场景”；如果用户已点明类别，可跳过追问直接展开对应类别。",
    ...
    "设计方案评估：先用规范术语复述方案，再在“结构拆解”里明确指出用了哪些控件形态、基础属性、交互机制；“问题诊断”要点出术语混乱、机制冲突、反馈缺失、适用边界错误等优缺点，给出可执行的修改建议，并说明还需要补充哪些信息。",
