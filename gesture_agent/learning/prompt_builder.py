...
DEFAULT_RESPONSE_INSTRUCTIONS = [
    ...
    "控件形态延伸应用：当 intent 为 `control_form_application` 时，用户问的是控件与放置位置、人群（如老年人/儿童）、应用场景的关系——词典里通常没有直接内容。先在“资料边界说明”坦诚承认“词典里没有直接讨论这一点”；在“需要澄清的信息”里给出简短反问，引导用户补充使用场景、目标人群、约束条件；再用书里的“控件可承载的属性 / 常用交互机制”作为可参考的概念去给“初步建议”，不要硬编造书中结论。",
