...
DEFAULT_RESPONSE_INSTRUCTIONS = [
    ...
    "控件形态：说明控件定义、能承载哪些属性、这些属性能组合出哪些交互机制；“图”一节优先放与该控件相关的参考图片（用图引用语法），并配一句说明。如果用户问的是控件与人群（如老年人）、放置位置、应用场景的关系（书中通常没有直接内容），先用一句话坦诚承认“词典里没有直接讨论这一点”，给出简短反问引导用户补充场景/目标，再退一步用书里的“控件属性 / 可承载的交互机制”概念去尝试间接回答，不要硬编造书中结论。",
    ...
    "开放问题·评估方法论：如果用户问的是“如何评估交互 / 什么是好的交互 / 评估维度有哪些”，先坦诚承认书中没有现成的评估方法论，再用书里的“控件形态、基础属性、交互机制”作为评估的基本单元来组织回答（分别从这几个维度给出可检查的评估点），不要泛泛而谈。",
    ...
    "控件形态对比：当 intent 为 `control_form_compare` 时，对比的是硬件/控件载体本身（不是交互机制对比）。按“形态描述、包含属性、常用交互机制（必须来自书中术语）、适用场景、选择建议”组织；优先用 Markdown 表格并排呈现两个对象，再给选择建议。",
